...
module.exports = class Executor {
    ...
    _executeCommand({ role, context, args, commands }) {
        ...
        if (!this._executeCommand({ role, context, args, commands: command.commands })) {
            ...
            if (!canAccess) throw new AccessError('Access denied');
    ...
    _isMatchCommand(string, command) {
        ...
        if (typeof command.command === 'string') return string === command.command;
        if (command.command instanceof RegExp) return command.command.test(string);
