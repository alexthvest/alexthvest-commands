...
module.exports = class Executor {
    ...
    _executeCommand(context, args, commands) {
        ...
        if (!this._executeCommand(context, args, command.commands)) {
            ...
            const params = this._processArgs(command, args, context);
            context      = context || {};

            this._checkAccess(command, params, context);
    ...
    _checkAccess(command, params, context) {
        ...
        const hasAccess = command.access(params, context);
     ...
     * @private
     */
    _processArgs(command, args, context) {
        const values = {};
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            const validate = param.validate(values[name], context);
